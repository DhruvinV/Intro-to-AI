...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.startingPosition,(None,)*4)
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    closest_corner = remaining_corners(state[0],state[1],problem)
    cost = 0
    xy1 = state[0]
    k = 0
    while(k < len(closest_corner)):
        min_dist = float("inf")
        min_cord = 0
        for i in closest_corner:
            xy3 = i
            if(xy1 != xy3):
                dis = abs(xy1[0] - xy3[0]) + abs(xy1[1] - xy3[1])
                if(dis < min_dist):
                    min_dist = dis
                    min_cord = xy3
        cost = cost + min_dist
...
def remaining_corners(curr_location,remaining_corners,problem):
    i = 0
    maxx_d = 0
    maxx_dloc = (0,0)
    xy2 = []
    while(i<4):
        # print("with remaining_corners",remaining_corners,remaining_corners[i]==None)
        if(remaining_corners[i]==None):
            xy2.append(problem.corners[i])
        i=i+1
    return xy2
